...
We adapted the Example_FunctionApproximatorDropout.py script and created an MLP with 
1 input node, 1 output node and 2 hidden layers, each with 50 nodes.
It is similar with the Chronis_Sakos_2_hidden_layer.py script with added lines
of code for dropout
Keep probability = 50 %
We used the Dropout in the 2nd hidden layer 
------------------------------------------------------------------------
...
print("Creating the second hidden layer with ", n_hidden_2, " nodes")       #2nd hidden layer
...
dmodel       = tf.nn.dropout(layer_2, keep_prob)            #dropout in the 2nd hidden layer
...
probabilities = tf.matmul(dmodel, output) + bias_output
...
if plotting:
    # plotting the function f(x)=sqr(x)
    plt.subplot(2, 1, 1)
